...
verifiable runtime decision chains; a property not satisfied by existing
hash-chain or standard Merkle-tree-based audit stores, which either lack
$O(\log n)$ inclusion proofs without full replay or require modifying
previously committed nodes on insertion. We present LICITRA-MMR, an open-source
...
contains 1,994 nodes and produces inclusion proofs requiring 14
hash operations. A standard balanced Merkle tree over the same leaf count
requires $\lceil\log_2(1000)\rceil = 10$ hashes per proof path but incurs
$O(\log n)$ internal node \emph{rewrites} on each insertion, since
sibling hashes along the rightmost path must be updated. The MMR trades a
slightly longer proof path for fully append-only insertion semantics: no
previously committed node is ever modified, making it strictly preferable
for tamper-evident audit logs where insertion integrity is paramount. The
...
\textit{Note: all tamper-evidence guarantees below are relative to at least
one honest anchor holder retaining a copy of a recent epoch hash --- a
trust assumption stated explicitly in Section~\ref{sec:trust} and
structurally equivalent to Certificate Transparency's honest log monitor
assumption.}
...
semantics, immudb is a mature alternative. However, immudb cannot be used
as-is for agentic governance for three reasons: (1)~it has no per-organization
epoch anchoring --- the entire database shares a single Merkle root, making
per-tenant forensic isolation difficult; (2)~it provides no canonical
serialization specification, meaning cross-implementation verification
requires out-of-band agreement on payload encoding; and (3)~it has no
integration path for pre-execution authorization semantics --- it records
whatever the application submits with no policy evaluation layer.
LICITRA-MMR is distinguished by its governance architecture: per-organization
epoch anchoring, versioned canonical JSON specification, and a designed
integration point for the LICITRA-SENTRY pre-execution control plane.
...
The figures in this section are \textbf{back-of-envelope analytical
estimates}, not measured benchmarks. They are derived from first principles
using known SHA-256 throughput characteristics and asymptotic node counts.
No profiling harness, database I/O, or network overhead is included.
Actual performance will vary depending on hardware, PostgreSQL
configuration, connection pool behaviour, payload size, and cache warmth.

\textit{Estimation basis:} Python~3.11 \texttt{hashlib.sha256} wrapping
OpenSSL on a commodity x86-64 processor; SHA-256 throughput assumed at
approximately 500~ns per call based on published OpenSSL benchmarks for
single-block inputs. Payload size assumed at approximately 200~bytes.
Database I/O latency dominates the commit path; cryptographic overhead is
analytically negligible in all scenarios. All numbers should be treated as
order-of-magnitude estimates pending a formal benchmark suite.
...
A formal microbenchmark suite measuring end-to-end commit latency, inclusion
proof generation, and epoch chain verification against a live PostgreSQL
instance --- including p50/p95 distributions, cold vs.\ warm cache
conditions, and database I/O breakdown --- is planned for v0.2.
...
committed accurately reflects what the agent was authorized to do: a
sufficiently privileged application layer could submit fabricated or
policy-bypassing events that the ledger would commit faithfully.

Closing this gap is the scope of LICITRA-SENTRY, the planned next layer of
the LICITRA governance stack. LICITRA-SENTRY intercepts agent actions
before they reach the commit pipeline, evaluates them against declarative
per-agent semantic contracts, and binds the authorization decision ---
approved or blocked --- to the MMR record in the same transaction. This
\emph{commitment before execution} property means the audit record contains
not just what the agent did, but cryptographic evidence that the action was
evaluated against policy at the moment of execution. LICITRA-SENTRY is
planned for v0.2; the present paper establishes the ledger primitive on
which that control layer depends.
...
All source code, test suites, and evidence bundles are publicly available.

\textbf{Repository:} \url{https://github.com/narendrakumarnutalapati/licitra-mmr-core}\\
\textbf{Tagged release:} \texttt{v0.1.0-mvp} (commit \texttt{70ec6e4})\\
\textbf{Zenodo archive (DOI):} \url{https://doi.org/10.5281/zenodo.18796529}\\
\textbf{Runtime:} Python~3.11, PostgreSQL~15, FastAPI, SQLAlchemy\\
\textbf{Test command:} \texttt{pytest tests/ -v} (11/11 passing)\\
\textbf{Evidence bundles:} Five reproducible JSON scenarios (S01--S05)
covering normal commit, leaf tamper, epoch tamper, row deletion, and
cross-epoch verification, included in the repository root.
...
\newblock In \textit{Advances in Cryptology -- CRYPTO~2025}, LNCS vol.~16001,
  pp.~170--202, Springer, 2025.
\newblock \doi{10.1007/978-3-032-01878-6\_6}
\newblock ePrint: \url{https://eprint.iacr.org/2025/234}
